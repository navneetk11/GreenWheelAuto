...
const getAllVehiclesWithRange = async (req, res) => {
    try {
        const [vehicles] = await db.query(`
            SELECT vid, name, brand, model, year, shape,
                   price, range_km, range_winter_km, 
                   charging_speed, drive_type
            FROM Item
...
const getTopRangeVehicles = async (req, res) => {
    try {
        ...
        const [vehicles] = await db.query(`
            SELECT vid, brand, model, price, range_km, charging_speed
            FROM Item
            WHERE range_km IS NOT NULL
            ORDER BY range_km DESC
            LIMIT ?
        ...
        res.status(500).json({ success: false, message: 'Failed to fetch top vehicles', error: error.message });
    }
};
...
const checkRangeSuitability = async (req, res) => {
    try {
        ...
        if (!dailyCommute || !drivingType) {
            return res.status(400).json({ success: false, message: 'Daily commute and driving type are required' });
        }

        const commuteKm = parseFloat(dailyCommute);
        if (isNaN(commuteKm) || commuteKm <= 0) {
            return res.status(400).json({ success: false, message: 'Please enter a valid daily commute distance' });
        }

        const [vehicles] = await db.query(`
            SELECT vid, name, brand, model, price, shape,
                   drive_type, range_km, range_winter_km, charging_speed
            FROM Item
            WHERE range_km IS NOT NULL
            ORDER BY range_km DESC
        `);
        ...
        if (vehicles.length === 0) {
            return res.status(404).json({ success: false, message: 'No vehicles found in database' });
        }
        ...
        const results = vehicles.map(vehicle => {
            const baseRange = vehicle.range_winter_km || vehicle.range_km;
        ...
        res.status(200).json({
            ...
            user_inputs: { daily_commute: commuteKm, driving_type: drivingType, home_charger: homeCharger || 'No' },
            data: sortedResults
        });

    } catch (error) {
        res.status(500).json({ success: false, message: 'Failed to check range suitability', error: error.message });
    }
};

module.exports = { getAllVehiclesWithRange, getRangeStatistics, getTopRangeVehicles, checkRangeSuitability };